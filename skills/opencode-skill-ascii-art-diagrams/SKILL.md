...
python draw_diagram.py | python <skill_base_dir>/scripts/verify.py

# Verify a specific diagram from a markdown file by heading:
python <skill_base_dir>/scripts/verify.py --extract "State Machine" < file.md

# Verify the Nth fenced code block (1-based):
python <skill_base_dir>/scripts/verify.py --block 3 < file.md

# Show column positions of structural chars on a specific line:
python <skill_base_dir>/scripts/verify.py --columns 5 < diagram.txt

# Quiet mode (only print failures):
python <skill_base_dir>/scripts/verify.py --quiet < diagram.txt
```
...
2. Write a   script using `grid.py` to construct the diagram
